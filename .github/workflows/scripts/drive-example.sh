...
then
  ...
  melos exec -c 1 --scope="$FLUTTERFIRE_PLUGIN_SCOPE_EXAMPLE" --dir-exists=web -- \
    flutter drive --device-id=chrome --test-arguments="--no-sound-null-safety" --enable-experiment=non-nullable --no-sound-null-safety --no-pub --verbose-system-logs --browser-name=chrome --target=./test_driver/MELOS_PARENT_PACKAGE_NAME_e2e.dart
  exit
fi
